      ...
      <div className="relative mx-auto max-w-6xl px-5 py-20 md:px-8 md:py-28">
        ...
        <ul className="mt-14 space-y-0">
          {posts.map((post, index) => {
            const showCover = hasSanityImage(post.coverImage as never);
            return (
              <li key={post._id}>
                <FadeIn delayMs={index * 60}>
                  <Link
                    href={`/blog/${post.slug}`}
                    className="group grid gap-6 border-t border-line py-8 transition hover:bg-paper/40 md:grid-cols-[180px_1fr] md:items-center"
                  >
                    {showCover ? (
                      ...
                      <div
                        className="hidden aspect-[4/3] rounded-xl bg-mist/80 md:block"
                        aria-hidden
                      />
                    )}
                    <div>
